...
for i in range(traj_count):
    ...
    true_cost_batch += [true_sys_sol['cost_opt']]
true_cost = np.mean(true_cost_batch)
...
for i in range(100):
    ...
    print(
        ...
        '| updated_model_cost:', np.mean(updated_model_cost_batch),
        # '| true_sys_cost:', np.mean(true_sys_cost_batch),
        '| updated_true_sys_cost:', np.mean(updated_true_sys_cost_batch),
    )
